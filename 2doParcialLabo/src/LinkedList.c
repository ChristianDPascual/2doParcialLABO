...
{//Agrega un elemento a la lista // pelements
...
int ll_clear(LinkedList* this)
{
    ...
    if(this!=NULL)
    {
    	...
    	while(tamanio>0 && ll_remove(this,tamanio-1)==0)
    	{
    		tamanio--;
    	}
    	returnAux=0;
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    tamanio=ll_len(this);
    if(this!=NULL && from>=0 && from<to && to<=tamanio)
    {
    	...
    	if(cloneArray!=NULL)
    	{
    		 for(i=from;i<to;i++)
    		 {
    		      ...
    		      if(addNode(cloneArray,index,auxiliar)==0)
    		      {
    		    	  index++;
    		      }
    		      else
